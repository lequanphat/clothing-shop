...
function FooterColumn({title, content = []}) {
    return (
        ...
        // </div>
        <div>
            <h1 className="text-[1.4rem] text-[#333] dark:text-[#ccc] font-bold mt-[40px] mb-[20px]">{title}</h1>
            {
                content.map((item, index) => <FooterItem key={index} item={item.title} icon={item.icon}/>)
            }
        </div>
    );
}
